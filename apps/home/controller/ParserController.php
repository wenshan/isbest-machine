    ...
    public function parserPositionLabel($content, $scode, $page = null, $link = null)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                $out_html = '<a href="' . SITE_INDEX_DIR . '/">' . $indextext . '</a><span class="nv-breadcrumb-delimiter">\</span>';
                if ($page && $scode == 0) {
                    $out_html .= $separator . '<a href="' . $link . '">' . $page . '</a><span class="nv-breadcrumb-delimiter">\</span>';
                } else {
                    foreach ($data as $key => $value) {
                        if ($value['outlink']) {
                            $out_html .= $separator . '<a href="' . $value['outlink'] . '">' . $value['name'] . '</a><span class="nv-breadcrumb-delimiter">\</span>';
                        } else {
                            $out_html .= $separator . '<a href="' . $this->parserLink($value['type'], $value['urlname'], 'list', $value['scode'], $value['filename'], '', '') . '">' . $value['name'] . '</a><span class="nv-breadcrumb-delimiter">\</span>';
                        }
                    }
                }
                // 执行内容替换
